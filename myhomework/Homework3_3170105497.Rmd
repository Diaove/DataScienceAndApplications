...
```
$$
SD=\sqrt{\frac{\sum^n_{i=1}(X_n-\overline{X})^2}{n-1}}
$$
...
sqrt(var_jackknifed)
```
Compared to part(a) `sd(gmp$pcgmp)=9219.663`,jack-knife approximation to the standard error of the mean is much smaller.
...
plm.jackknife<-function(a,N = gmp$pop,Y = gmp$pcgmp){
  ...
  for (i in 1:length(y0)) {
    y0.means = c(y0.means,mean_i(i,Y=y0))
  }
  sd_y0=(n-1)/sqrt(n)*sd(y0.means)
  for (i in 1:length(aa)) {
    aa.means = c(aa.means,mean_i(i,Y=aa))
  }
  sd_aa=(n-1)/sqrt(n)*sd(aa.means)
...
The `y0_fin=6611` calculated by plm is basically the same as the initial value, `a_fin=0.1433688` is slightly different from the initial value, the standard deviation of `y0` is 3.439384e-11, and the standard deviation of `a` is 2.713581e-06.
